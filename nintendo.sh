...
main(){
    ...
    if [ "$1" = "z" ]; then
        if [ -z "$2" ]; then
            echo 'You need to pass a valid argument'
            exit 1
        else
            zelda "$@"
        fi
        ...
        if [ "$1" = "b" ]; then
            ...
            if [ "$1" = "p" ]; then
                ...
                echo 'You need to pass a valid argument. Valid arguments are: z, b or p'
                exit 1
            fi
        fi
    fi
...
zelda(){
    curl -s https://botw-compendium.herokuapp.com/api/v2/entry/"$2" | jq -r '. | "\n\("===================")\n\("Basic Information")\n\("===================")\n\("Name: ")\(.data.name)\n\("ID: ")\(.data.id)\n\("Category: ")\(.data.category)\n\("Description: ")\(.data.description)\n\("===================")\n\("Common Locations")\n\("===================")\n\("Location 1: ")\(.data.common_locations[0])\n\("Location 2: ")\(.data.common_locations[1])\n\("===================")\n\("Drops")\n\("===================")\n\("Drop 1: ")\(.data.drops[0])\n\("Drop 2: ")\(.data.drops[1])\n\("Drop 3: ")\(.data.drops[2])\n"'
}

pokemon(){
    curl -s https://pokeapi.co/api/v2/pokemon/"$2" | jq -r '. | "\n\("===================")\n\("Basic Information")\n\("===================")\n\("Pokemon: ")\(.name)\n\("ID: ")\(.id)\n\("Type 1: ")\(.types[0].type.name)\n\("Type 2: ")\(.types[1].type.name)\n\("===================")\n\("Base Stats")\n\("===================")\n\("HP: ")\(.stats[0].base_stat)\n\("Attack: ")\(.stats[1].base_stat)\n\("Defense: ")\(.stats[2].base_stat)\n\("Special Attack: ")\(.stats[3].base_stat)\n\("Special Defense: ")\(.stats[4].base_stat)\n\("Speed: ")\(.stats[5].base_stat)"'
}

poke_battle(){
    poke1="$(curl -s https://pokeapi.co/api/v2/pokemon/"$2" | jq -r '.types[0].type.name')"
    poke2="$(curl -s https://pokeapi.co/api/v2/pokemon/"$3" | jq -r '.types[0].type.name')"
